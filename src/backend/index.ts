...
export async function isToolKeyAuthentic(key: string) {
  ...
  return !!(await (await getDb())
    .collection<WithId<ApiKey>>('tool-keys')
    .find({ key })
    .limit(1)
    .count());
}
...
export async function getNoFlyList() {
  return (await getDb())
    .collection<NoFlyListEntry>('no-fly-list')
    .find()
    .sort({ id: 1 })
    .project({
      _id: false
...
export async function getAirports() {
  return (await getDb())
    .collection<InternalAirport>('airports')
    .find()
    .sort({ id: 1 })
    .project({
      _id: false,
...
export async function getAirlines() {
  return (await getDb())
    .collection<InternalAirline>('airlines')
    .find()
    .sort({ id: 1 })
    .project({
      _id: false
...
export async function getExtras() {
  return (await (await getDb()).collection<WithId<InternalInfo>>('info').findOne({}))
    ?.allExtras;
}
...
export async function getFlightsById(params: GetFliByIdParams) {
  ...
  return (await getDb())
    .collection<WithId<InternalFlight>>('flights')
...
export async function searchFlights(params: SeaFliParams) {
  ...
  try {
    ...
    if (regexMatch.flight_id) {
      regexMatchObjectIds = regexMatch.flight_id
        .toString()
        .split('|')
  ...
  return (await getDb())
    .collection<InternalFlight>('flights')
    .aggregate<PublicFlight>(pipeline)
    .toArray();
}
...
export async function generateFlights(silent = false) {
  ...
  const airports = await db
    .collection<WithId<InternalAirport>>('airports')
    .find()
    .toArray();
  const airlines = await db
    .collection<WithId<InternalAirline>>('airlines')
    .find()
    .toArray();
  ...
  if (airports.length < 2 || airlines.length < 2)
    throw new FlightGenerationError(
      'cannot generate flights without at least two airports and airlines'
    );
  ...
  const hourLevelMsDilation = (epoch: number) =>
    Math.floor(epoch / oneHourInMs) * oneHourInMs;
  ...
  !silent &&
    console.info(
      `api   - deleted ${deleteResult.deletedCount} flights older than 7 days`
    );

  // ? Determine how many hours (if any) need flights generated for them
  const lastFlightId =
    (await flightDb.find().sort({ _id: -1 }).limit(1).next())?._id ?? new ObjectId();
  const lastFlightHourMs = hourLevelMsDilation(lastFlightId.getTimestamp().getTime());
  const totalHoursToGenerate =
    (hourLevelMsDilation(Date.now() + targetDaysInMs) - lastFlightHourMs) / oneHourInMs;

  if (totalHoursToGenerate <= 0) {
    // eslint-disable-next-line no-console
    !silent &&
      console.info(
        `api   - No flights need to be generated yet (${totalHoursToGenerate}). Terminated.`
      );
  ...
  const gatePool = 'abcdefghijklmnopqrstuvwxyz'
    .split('')
    .slice(0, getEnv().AIRPORT_NUM_OF_GATE_LETTERS)
    .map((x) => {
      return [...Array(getEnv().AIRPORT_GATE_NUMBERS_PER_LETTER)].map(
        (_, n) => `${x}${n + 1}`
      );
    })
    .flat();
  ...
  [...Array(totalHoursToGenerate)].forEach((_, i) => {
    if (chance() > getEnv().FLIGHT_HOUR_HAS_FLIGHTS_PERCENT) return;

    const currentHour = lastFlightHourMs + oneHourInMs + i * oneHourInMs;
    const activeAirlines = shuffle(airlines).slice(
      0,
      randomInt(2, airlines.length)
    ) as WithId<InternalAirline>[];
    const getFlightNum = activeAirlines.reduce<{ [objectId: string]: () => number }>(
      (map, airline) => {
        return {
          ...map,
    ...
    !silent &&
      console.info(
        `api   ↳ generating flights for hour ${currentHour} (${
          i + 1
        }/${totalHoursToGenerate})`
      );
    ...
    airports.forEach((firstAirport) => {
      ...
      airports.forEach((secondAirport) => {
        ...
        if (
          firstAirport._id.equals(secondAirport._id) ||
          chance() > getEnv().AIRPORT_PAIR_USED_PERCENT
        )
          return;
        ...
        activeAirlines.forEach((airline) => {
          ...
          const numSeats = [...Array(info.seatClasses.length)]
            .map((_) => {
              return randomInt(
                MIN_SEATS_PER_PLANE,
                SEATS_PER_PLANE / info.seatClasses.length
              );
            })
            .sort((a, b) => b - a);
          ...
          for (const [ndx, seatClass] of Object.entries(info.seatClasses)) {
            ...
            prevSeat$ =
              randomInt(prevSeat$, prevSeat$ * 2) + Number(Math.random().toFixed(2));
          ...
          for (const item of info.allExtras) {
            ...
            prevItem$ =
              randomInt(prevItem$, prevItem$ * 2.5) + Number(Math.random().toFixed(2));
          ...
          statelessFlights.push({
            ...
            flightNumber:
              airline.codePrefix + getFlightNum[airline._id.toHexString()]().toString(),
            baggage: {
              checked: {
                max: maxChecked,
      ...
      statelessFlights.forEach((flight) => {
        ...
        if (recentState.status == 'cancelled') return;
        ...
        for (let stage = 6; !done && stage < 10; ++stage) {
          ...
          switch (stage) {
            ...
            case 7:
              if (isArrival)
                throw new GuruMeditationError(
                  'arrival type encountered in departure-only model'
                );

              // ? This flight has started boarding
              prevActiveAfter =
                state.arriveAtReceiver + randomInt(threeMinutesInMs, tenMinutesInMs);
            ...
            case 8:
              ...
              if (!state.departFromReceiver)
                throw new GuruMeditationError(
                  'illegal departure state encountered in model (1)'
                );
            ...
            case 9:
              ...
              if (!state.departFromReceiver)
                throw new GuruMeditationError(
                  'illegal departure state encountered in model (2)'
                );

              prevActiveAfter =
                state.departFromReceiver + randomInt(2 * oneHourInMs, 5 * oneHourInMs);
              state.status = 'past';
              state.gate = null;
              break;
  ...
  try {
    ...
    if (!operation.result.ok) throw new FlightGenerationError('flight insertion failed');

    if (operation.insertedCount != flights.length)
      throw new GuruMeditationError(
        'assert failed: operation.insertedCount != totalHoursToGenerate'
      );
