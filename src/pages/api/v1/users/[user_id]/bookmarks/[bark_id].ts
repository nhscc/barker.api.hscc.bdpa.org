...
import { bookmarkBark, isBarkBookmarked, unbookmarkBark } from 'universe/backend';
    ...
    async ({ req, res }) => {
      ...
      if (bark_id !== undefined) {
        ...
        if (user_id !== undefined) {
          if (req.method == 'GET') {
            (await isBarkBookmarked({ key, bark_id, user_id }))
              ? sendHttpOk(res)
              : sendHttpNotFound(res);
          } else if (req.method == 'DELETE') {
            await unbookmarkBark({ key, bark_id, user_id });
            sendHttpOk(res);
          } else {
            await bookmarkBark({ key, bark_id, user_id });
            sendHttpOk(res);
          }
        }
      }
