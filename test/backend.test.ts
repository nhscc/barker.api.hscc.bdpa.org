...
describe('universe/backend', () => {
  describe('::getNoFlyList', () => {
    it('returns the No Fly List data as expected', async () => {
      ...
      expect(await Backend.getNoFlyList()).toStrictEqual(
        unhydratedDummyDbData.noFlyList.map((nfl) => {
  ...
  describe('::getAirports', () => {
    it('returns the airport adhering to the PublicAirport type', async () => {
      expect.hasAssertions();
      expect(await Backend.getAirports()).toStrictEqual(
        unhydratedDummyDbData.airports.map((airport) => {
          // @ts-expect-error: checking for existence of _id
  ...
  describe('::getAirlines', () => {
    it('returns the airline data as expected', async () => {
      expect.hasAssertions();
      expect(await Backend.getAirlines()).toStrictEqual(
        unhydratedDummyDbData.airlines.map((airline) => {
          // @ts-expect-error: checking for existence of _id
  ...
  describe('::getExtras', () => {
    it('returns the extras data as expected', async () => {
      expect.hasAssertions();
      expect(await Backend.getExtras()).toStrictEqual(
        unhydratedDummyDbData.info.allExtras
      );
    });
  });
  ...
  describe('::getSeats', () => {
    it('returns the seats data as expected', async () => {
      expect.hasAssertions();
      expect(await Backend.getSeats()).toStrictEqual(
        unhydratedDummyDbData.info.seatClasses
      );
    });
  });
  ...
  describe('::getApiKeys', () => {
    it('returns the airline data as expected', async () => {
      ...
      expect(await Backend.getApiKeys()).toStrictEqual(
        unhydratedDummyDbData.keys.map((key) => {
          // @ts-expect-error: checking for existence of _id
  ...
  describe('::getFlightsById', () => {
    ...
    it('returns only public flight data when correct ids are passed', async () => {
      ...
      const result1 = await Backend.getFlightsById({
        ids: [flight1._id, flight2._id],
        key
      });
      ...
      expect(
        result1.every((flight) => {
          ...
          return (
            keys.every((key) => PFlightKeys.includes(key)) &&
  ...
  describe('::searchFlights', () => {
    ...
    it('search returns expected paginated records with empty asc/desc queries', async () => {
      ...
      expect(
        result1.every((flight) =>
          Object.keys(flight).every((key) => PFlightKeys.includes(key))
        )
      ).toBeTrue();
    });
    ...
    it('search returns expected paginated records with secondary matchers', async () => {
      ...
      const expectedFlights1 = expFlights
      ...
      const expectedFlights2 = expFlights
        ...
        .slice(-count)
        .reverse();
      const expectedFlights3 = expFlights
  ...
  describe('::addToRequestLog', () => {
    it('adds request to log as expected', async () => {
      ...
      const { _id: ignored1, ...log1 } =
        (await reqlog.findOne({ resStatusCode: 1111 })) || {};
      const { _id: ignored2, ...log2 } =
        (await reqlog.findOne({ resStatusCode: 2222 })) || {};
  ...
  describe('::isDueForContrivedError', () => {
    it('returns true after REQUESTS_PER_CONTRIVED_ERROR invocations', async () => {
      ...
      expect(
        [...Array(rate * 2)].map(() => Backend.isDueForContrivedError())
      ).toStrictEqual([
